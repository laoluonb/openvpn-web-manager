...
class InstallerRegressionTests(unittest.TestCase):
    ...
    def test_runtime_sync_management_socket_and_online_updater_are_installed(self) -> None:
        ...
        self.assertIn('APP_VERSION="1.2.14-test"', installer)
        ...
        self.assertIn('INSTALL_LOG="/var/log/openvpn-manager/update-install.log"', updater)
        self.assertIn('2>&1 | tee -a "$INSTALL_LOG"', updater)
        self.assertIn('INSTALL_EXIT_CODE=${PIPESTATUS[0]}', updater)
        self.assertIn('FAILURE_DETAIL=', updater)
        self.assertIn('OPENVPN_MANAGER_UPDATE=1 /bin/bash "$SOURCE_DIR/install.sh"', updater)
        self.assertIn('管理面板 $TARGET_VERSION 与 OpenVPN 均已是最新版本', updater)
        self.assertIn('apt-get install -y --only-upgrade openvpn', updater)
